...
#define MPDEC_TAG			"mpdecision_hotplug"
...
static struct cpu_hotplug {
	...
	unsigned int mpdecision_enabled;
	unsigned int hotplug_suspend;
	struct mutex mpdecision_hotplug_mutex;
	struct mutex mpdecision_cpu_mutex;
} hotplug = {
	...
	.mpdecision_enabled = HOTPLUG_ENABLED,
	.hotplug_suspend = 1,
};
...
static void __ref mpdecision_hotplug_work(struct work_struct *work) {
	unsigned int cpu;

	if (hotplug.suspended && hotplug.max_cpus_online_susp <= 1)
		goto out;

	if (!mutex_trylock(&hotplug.mpdecision_cpu_mutex))
		goto out;
	...
	mutex_unlock(&hotplug.mpdecision_cpu_mutex);
...
static void mpdecision_hotplug_suspend(struct work_struct *work)
{
	int cpu;

	if (!hotplug.mpdecision_enabled || hotplug.suspended)
		return;

	if (!hotplug.hotplug_suspend)
		return;

	mutex_lock(&hotplug.mpdecision_hotplug_mutex);
	hotplug.suspended = 1;
	...
	mutex_unlock(&hotplug.mpdecision_hotplug_mutex);
	...
	pr_info(MPDEC_TAG": Screen -> off. Deactivated mpdecision hotplug. | Mask=[%d%d%d%d]\n",
			cpu_online(0), cpu_online(1), cpu_online(2), cpu_online(3));
}

static void __ref mpdecision_hotplug_resume(struct work_struct *work)
{
	...
	if (hotplug.suspended) {
		...
		if (hotplug.max_cpus_online_susp <= 1) {
			...
			INIT_DELAYED_WORK(&hotplug_work, mpdecision_hotplug_work);
		}
	}
	...
	if (required_reschedule) {
		...
		pr_info(MPDEC_TAG": Screen -> on. Activated mpdecision hotplug. | Mask=[%d%d%d%d]\n",
				cpu_online(0), cpu_online(1), cpu_online(2), cpu_online(3));
	}
}

#ifdef CONFIG_POWERSUSPEND
static void __mpdecision_hotplug_suspend(struct power_suspend *handler)
{
	INIT_DELAYED_WORK(&suspend_work, mpdecision_hotplug_suspend);
	mod_delayed_work_on(0, susp_wq, &suspend_work,
			msecs_to_jiffies(hotplug.suspend_defer_time * 1000));
}

static void __ref __mpdecision_hotplug_resume(struct power_suspend *handler)
{
...
static struct power_suspend mpdecision_hotplug_power_suspend_driver = {
	.suspend = __mpdecision_hotplug_suspend,
	.resume = __mpdecision_hotplug_resume,
};
#else
...
static int mpdecision_hotplug_start(void)
{
	int cpu, ret = 0;
	struct down_lock *dl;

	hotplug_wq = alloc_workqueue("mpdecision_hotplug", WQ_HIGHPRI | WQ_FREEZABLE, 0);
	if (!hotplug_wq) {
		ret = -ENOMEM;
		goto err_out;
	}
...
#ifdef CONFIG_POWERSUSPEND
	register_power_suspend(&mpdecision_hotplug_power_suspend_driver);
#else
	...
	mutex_init(&hotplug.mpdecision_cpu_mutex);
	mutex_init(&hotplug.mpdecision_hotplug_mutex);

	INIT_DELAYED_WORK(&hotplug_work, mpdecision_hotplug_work);
	INIT_DELAYED_WORK(&suspend_work, mpdecision_hotplug_suspend);
	INIT_WORK(&resume_work, mpdecision_hotplug_resume);
...
err_out:
	hotplug.mpdecision_enabled = 0;
	return ret;
}

static void mpdecision_hotplug_stop(void)
{
	int cpu;
	struct down_lock *dl;
	...
	cancel_delayed_work_sync(&suspend_work);
	cancel_delayed_work_sync(&hotplug_work);
	mutex_destroy(&hotplug.mpdecision_hotplug_mutex);
	mutex_destroy(&hotplug.mpdecision_cpu_mutex);
#ifdef CONFIG_POWERSUSPEND
	unregister_power_suspend(&mpdecision_hotplug_power_suspend_driver);
#else
...
(struct device *dev, struct device_attribute *mpdecision_hotplug_attrs,	\
 char *buf)								\
{									\
...
show_one(mpdecision_enabled, mpdecision_enabled);
...
static ssize_t show_idle_freq (struct device *dev,
				struct device_attribute *mpdecision_hotplug_attrs,
				char *buf)
{
...
static ssize_t store_mpdecision_enabled(struct device *dev,
				struct device_attribute *mpdecision_hotplug_attrs,
				const char *buf, size_t count)
{
	unsigned int input;
	...
	if (input == hotplug.mpdecision_enabled)
		return count;

	hotplug.mpdecision_enabled = input;

	if (!hotplug.mpdecision_enabled) {
		state = MSM_MPDEC_DISABLED;
		mpdecision_hotplug_stop();
		pr_info(MPDEC_TAG": Disabled\n");
	} else {
		state = MSM_MPDEC_IDLE;
		mpdecision_hotplug_start();
...
static DEVICE_ATTR(enabled, 644, show_mpdecision_enabled, store_mpdecision_enabled);
static DEVICE_ATTR(hotplug_suspend, 644, show_hotplug_suspend, store_hotplug_suspend);

static struct attribute *mpdecision_hotplug_attrs[] = {
...
static struct attribute_group attr_group = {
	.attrs = mpdecision_hotplug_attrs,
	.name = "conf",
};

/**************************** SYSFS END ****************************/

static int mpdecision_hotplug_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct kobject *mpdecision_kobj;
	...
	ret = sysfs_create_group(mpdecision_kobj,
			&attr_group);

        if (ret) {
		pr_err("%s mpdecision_kobj create failed!\n",
			__func__);
		goto err_dev;
	}

	if (hotplug.mpdecision_enabled) {
		ret = mpdecision_hotplug_start();
		if (ret != 0)
			goto err_dev;
	}
	...
	return ret;
err_dev:
	if (mpdecision_kobj != NULL)
		kobject_put(mpdecision_kobj);
	return ret;
}

static struct platform_device mpdecision_hotplug_device = {
	.name = MPDEC_TAG,
	.id = -1,
};

static int mpdecision_hotplug_remove(struct platform_device *pdev)
{
	if (hotplug.mpdecision_enabled)
		mpdecision_hotplug_stop();
...
static struct platform_driver mpdecision_hotplug_driver = {
	.probe = mpdecision_hotplug_probe,
	.remove = mpdecision_hotplug_remove,
	.driver = {
		.name = MPDEC_TAG,
		.owner = THIS_MODULE,
	},
...
static int __init msm_mpdec_init(void)
{
	...
	ret = platform_driver_register(&mpdecision_hotplug_driver);
	if (ret) {
		pr_err("%s: Driver register failed: %d\n", MPDEC_TAG, ret);
		return ret;
	}

	ret = platform_device_register(&mpdecision_hotplug_device);
	if (ret) {
...
{
	platform_device_unregister(&mpdecision_hotplug_device);
	platform_driver_unregister(&mpdecision_hotplug_driver);
}
...
MODULE_DESCRIPTION("MPDecision Hotplug Driver");
